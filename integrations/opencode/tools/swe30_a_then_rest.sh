...
# Strategy A: same server binary, same envelope, the per-request
# chat-completions inferlet instead of the session daemon. The A-vs-B
# comparison the four-way lacked; requested 2026-08-18 so B's robustness
# report has its in-family baseline. Ingress ignores the model id, so the
# probe model string works unchanged.
PIEA_RESTART="pkill -f '$REPO/target/release/pie .*serve'; pkill -f session_shim.py; sleep 10; \
$MEMWAIT; \
PIE_PYTHON=$PIEPY $REPO/integrations/opencode/tools/boot_pie.sh s36 \
...
log "=== CONTINUATION: strategy A first, then mlx/vllm/llamacpp (pie-B already in $OUT) ==="
log "instances: $(echo $INSTANCES | wc -w | tr -d ' ')  budget: ${BUDGET}s  no-think: $NOTHINK"

arm pie-a    http://127.0.0.1:8080 "probe/$PIEMODEL"  "$PIEA_RESTART"
...
for a in pie pie-a mlx vllm llamacpp; do
