...
def emit_fixtures(a, arms, shapes, AutoTokenizer):
    """Record what the oracle says, so `cargo test` can check it without Python.

    This is the regeneration half of the same trade
    `model/config/tests/differential.rs` makes against the C++ normalizer: the
    foreign toolchain runs when a human asks, and the PR path only ever reads
    checked-in bytes.

    Each case carries the request BODY as well as the expected ids, so the Rust
    side needs no second copy of the shape table -- a copy that could drift.
    """
    out = pathlib.Path(a.emit_fixtures)
    ...
    for arm, cache_dir, deploy_name, arch, *rest in arms:
        ...
        tmp = out / "tok" / f".{arm}.tmp"
        proc = subprocess.run([a.bin, "--compile", str(snap / "tokenizer.json"), str(tmp)],
                              capture_output=True, text=True)
        if proc.returncode != 0:
            print(f"  FAILED to compile {arm}: {proc.stderr[:300]}", file=sys.stderr)
            return 1
        digest = hashlib.sha256(tmp.read_bytes()).hexdigest()[:16]
        final = out / "tok" / f"{digest}.pietok"
        if final.exists():
            tmp.unlink()
        else:
            tmp.rename(final)
        ...
        for name, (messages, tools) in shapes.items():
            for thinking in (False, True):
                ...
                if tools:
                    body["tools"] = tools
                cases.append({"arm": arm, "shape": name, "thinking": thinking,
                              "body": body, "ids": ids})

    doc = {
        "note": (
            "Generated by check_render_matrix.py --emit-fixtures. The oracle is "
            "HuggingFace apply_chat_template on each checkpoint's own "
            "chat_template.jinja; these ids are what it produced. Regenerate "
            "deliberately and review the diff -- a change here means a template "
            "moved, which is news. Never edit by hand to make a test pass."
        ),
        "tokenizers": tokenizers,
        "arms": arm_meta,
        "cases": cases,
    }
    (out / "cases.json").write_text(json.dumps(doc, indent=1, ensure_ascii=False))
    total = sum((out / p).stat().st_size for p in tokenizers.values())
    print(f"  {len(cases)} cases, {len(tokenizers)} tokenizer blob(s), "
...
def main():
    ...
    ap.add_argument("--emit-fixtures", metavar="DIR",
                    help="write committed fixtures (compiled tokenizers + "
                         "expected token ids) so CI can check parity with no "
                         "Python and no HF cache")
    ...
    if a.emit_fixtures:
        return emit_fixtures(a, arms, shapes, AutoTokenizer)
