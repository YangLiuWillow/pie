...
DeviceTuning tuning_for(const DeviceInfo& info) {
    ...
    t.sdpa_unroll = env_bool("PIE_METAL_SDPA_UNROLL", t.sdpa_unroll);
    t.sdpa_nax = env_bool("PIE_METAL_SDPA_NAX", t.sdpa_nax);
    t.qmm_nax = env_bool("PIE_METAL_QMM_NAX", t.qmm_nax);
    t.sdpa_nax_min_rows =
        env_int("PIE_METAL_SDPA_NAX_MIN_ROWS", t.sdpa_nax_min_rows);
...
bool sdpa_unroll() { return device_tuning().sdpa_unroll; }
