...
# anywhere. Override REPO_ROOT to point at a different checkout.
REPO_ROOT=${REPO_ROOT:-$(cd "$SCRIPT_DIR/../.." && pwd)}

if [ "$BACKEND" = "pie-session" ]; then
    INFERLET_DIR=$REPO_ROOT/inferlets/openhands-coder-session
    WASM=$INFERLET_DIR/target/wasm32-wasip2/release/openhands_coder_session.wasm
    INFERLET_NAME="openhands-coder-session@0.1.0"
else
    INFERLET_DIR=$REPO_ROOT/inferlets/openhands-completion
    WASM=$INFERLET_DIR/target/wasm32-wasip2/release/openhands_completion.wasm
