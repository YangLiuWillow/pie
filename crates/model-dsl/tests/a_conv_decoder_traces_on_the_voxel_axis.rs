...
impl Decoder {
    fn new() -> Decoder {
        Decoder {
            conv1: Weight::sym("conv1", [u64::from(C_MID), u64::from(C_IN) * 27], Dtype::Bf16)
            ...
            conv2: Weight::sym("conv2", [u64::from(C_OUT), u64::from(C_MID) * 27], Dtype::Bf16)
...
impl ForwardHybrid for Decoder {
    ...
    fn forward(&self, inputs: Input<NoFacts>) -> Value {
        ...
        assert_eq!(h.rows(), Dim::VoxelsTimes(4), "an upsample grows rows by its volume");
        let (h, g3) = spatial::conv3d(&h, &g2, &self.conv2, Some(&self.b2), Conv::same3(), None);
        let (y, g4) = spatial::pixel_shuffle(&h, &g3, [1, 2, 2]);
        assert_eq!(y.rows(), Dim::VoxelsTimes(16), "a shuffle grows rows by its block");
...
fn the_decoder_traces_and_its_grids_are_values() {
    ...
    for param in &trace.params {
        let want = if param.name == "conv1" {
            ParamLayout::ConvTapsMajor { c_in: C_IN, taps: 27 }
...
impl ForwardHybrid for Patchifier {
    ...
    fn forward(&self, inputs: Input<NoFacts>) -> Value {
        ...
        assert_eq!(tokens.rows(), Dim::Tokens, "patchify lands on the token axis");
...
impl ForwardHybrid for NaturalWeight {
    ...
    fn forward(&self, inputs: Input<NoFacts>) -> Value {
        let g = inputs.grid();
        let x = inputs.voxels(0, C_IN, Dtype::Bf16);
        let w = Weight::sym("conv", [u64::from(C_MID), u64::from(C_IN) * 27], Dtype::Bf16);
