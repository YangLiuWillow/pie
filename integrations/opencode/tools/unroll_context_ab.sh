...
# PROVE `REPO` IS THE REPO BEFORE ANYTHING USES IT.
#
# `stop_all` kills by "$REPO/target/release/pie .*serve". If REPO is wrong the
# pattern matches nothing, every kill silently no-ops, and the previous arm's
# server keeps its 22 GB — which is how a copy of this script run from /tmp
# resolved REPO to "/" (dirname /tmp/x.sh/../.. = /), left round 1's server
# alive, and drove the machine to 21.9 GB free until the preflight refused to
# continue. The guard caught it; the harness should not have needed the guard.
#
# A silently-wrong kill pattern is this project's most-repeated bug —
# `boot_pie.sh` exists because `pkill -f "pie serve"` never matched the real
# argv (`pie -c <config> serve`). Check the path, loudly.
if [ ! -x "$REPO/target/release/pie" ]; then
    echo "FATAL: PIE_REPO='$REPO' has no target/release/pie." >&2
    echo "       stop_all would kill nothing and the arms would fight for memory." >&2
    echo "       Run from the repo, or set PIE_REPO explicitly." >&2
    exit 1
fi
mkdir -p "$OUT"
cd "$REPO/integrations/opencode" || {
    echo "FATAL: no $REPO/integrations/opencode" >&2; exit 1; }
