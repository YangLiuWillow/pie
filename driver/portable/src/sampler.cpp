...
void run_distribution(const float* logits, std::int32_t vocab_size,
    ...
    // returned are the model's pre-softmax logits, not probabilities
    // (SDK `TopLogits` probe). A softmax at temperature 0 is degenerate,
    // so the sentinel claims no meaningful behavior.
