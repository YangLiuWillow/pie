...
fn cuda_budgets(
    ...
    // `max_model_len` when stated; else the contract's default. Beyond
    // capping one sequence, this sizes the arming pass's synthetic lanes, so
    // it decides which decode widths get an armed body on a small pool.
