...
//!
//! Port of the pre-rewrite `chat-completions/src/session.rs` (itself a port
//! of `codex-responses/src/session.rs`) to the chat message model. qwen-code
//! talks the stateless chat API: every turn re-sends the whole conversation.
//! After generating a turn we retain the working set under a key that is a
//! hash of the *message list a follow-up request would arrive with*
//! (everything up to and including the assistant turn we just produced).
//! The next request strips its trailing tool-result / new-user messages,
//! hashes the remainder, and — on a hit — resumes from the retained working
//! set, paying prefill only for the stripped suffix and the new cue.
//!
//! This module is the pure part: canonicalization, addressing, and the
//! resume-point split. Storage changed in the port: the daemon is now a
//! long-lived process, so retained `WorkingSet`s live in a process-local map
//! (see `handler.rs`) instead of engine-named snapshots — token-exact resume
//! (generated ids, sub-page tails) survives only in-process, and a daemon
//! restart degrades to a clean full-rebuild miss. A hash miss is always safe.
...
/// stale prefix is degraded output for one conversation, and a miss is a
/// full rebuild.
...
/// Hash of (tool schemas, no-think flag, canonical items) → session key.
///
...
/// including the last assistant message can have been retained by the
...
/// message at all (first turn — nothing could have been retained).
